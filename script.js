...
// ─── O/L Results reveal ────────────────────────────────────────────────────────
function initOLReveal() {
  const revealBtn  = document.getElementById("olRevealBtn");
  const collapseBtn = document.getElementById("olCollapseBtn");
  const panel      = document.getElementById("olResultsPanel");
  ...
  revealBtn.addEventListener("click", () => {
    ...
    // Animate each ol-card in sequence once the panel opens
    panel.querySelectorAll(".ol-card").forEach((card) => {
      const delay = card.style.getPropertyValue("--delay") || "0ms";
      card.style.transitionDelay = delay;
      card.classList.add("card-show");
    });

    // Smooth scroll to the panel after a brief moment
    setTimeout(() => {
      panel.scrollIntoView({ behavior: "smooth", block: "start" });
    }, 120);

    // Re-init card tilt for the newly revealed cards
    initCardTilt();
    lucide.createIcons();
  });
  ...
  collapseBtn.addEventListener("click", () => {
    ...
    // Scroll back up to the teaser
    document.getElementById("olTeaser").scrollIntoView({ behavior: "smooth", block: "start" });
  });
}
...
// ─── Live Age ──────────────────────────────────────────────────────────────────
function initAge() {
  // Birth timestamp — never exposed in the DOM
  const BIRTH = new Date("2009-07-09T00:00:00").getTime();
  ...
  function update() {
    const { y, mo, d, h, mi, s } = computeAge();
    el.innerHTML =
      `<span class="age-val">${y}</span><span class="age-unit">y</span>` +
      `<span class="age-sep">·</span>` +
      `<span class="age-val">${mo}</span><span class="age-unit">mo</span>` +
      `<span class="age-sep">·</span>` +
      `<span class="age-val">${d}</span><span class="age-unit">d</span>` +
      `<span class="age-sep">·</span>` +
      `<span class="age-val">${String(h).padStart(2,"0")}</span><span class="age-unit">h</span>` +
      `<span class="age-sep">·</span>` +
      `<span class="age-val">${String(mi).padStart(2,"0")}</span><span class="age-unit">m</span>` +
      `<span class="age-sep">·</span>` +
      `<span class="age-val age-sec">${String(s).padStart(2,"0")}</span><span class="age-unit">s</span>`;
  }
  ...
  const observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        entry.target
          .querySelectorAll(".info-card, .skill-card, .project-card, .ol-card")
          .forEach((card) => {
            const delay = card.style.getPropertyValue("--delay") || "0ms";
            card.style.transitionDelay = delay;
            card.classList.add("card-show");
...
function initCardTilt() {
  ...
  const cards = document.querySelectorAll(".hover-glow, .ol-card");
